/* ============================================================
   PédiatrieQuest — logique de jeu (gamification)
   Soins infirmiers pédiatriques — moteur cloné de SASIQuest
   ============================================================ */

const STORAGE_KEY = "pediatriequest_state_v1";
...
/* ------------------ Changer de totem en cours de parcours ------------------
   ...
   (l'identifiant stable est pediatriequest_device_id) : le changer à mi-
...
const SYNC_KEY = "pediatriequest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("pediatriequest_device_id");
  if (!id) {
    id = "pdq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("pediatriequest_device_id", id);
  }
  return id;
}
